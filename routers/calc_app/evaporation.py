...
from calculation_methods import calc_liguid_evaporation
...
async def get_evaporation_lpg(request: Request):
...
async def post_evaporation_lpg (request: Request,
...
@router.get('/liguid', response_class=HTMLResponse)
async def get_evaporation_liquid(request: Request):
    user = await get_current_user(request)
    if user is None:
        return RedirectResponse(url='/auth', status_code=status.HTTP_302_FOUND)
    return templates.TemplateResponse('calc_app/evaporation_liquid_form.html', {'request': request, 'user': user})


@router.post('/liguid', response_class=HTMLResponse)
async def post_evaporation_liquid (request: Request,
                           molar_mass: str = Form(...), strait_area: str = Form(...),
    ...
    # 1. Получим класс для расчета взрыва
    evaporation_unit = calc_liguid_evaporation.Liquid_evaporation()
    # 2. Получим зоны для большой таблицы (не классифицированные)
    zone_array = evaporation_unit.evaporation_array(float(molar_mass), float(strait_area), float(steam_pressure))
    # Преобразование к виду [(t, mass),...]
    zipped_values = zip(zone_array[0], zone_array[1])
    zipped_list = list(zipped_values)
    ...
    return templates.TemplateResponse('calc_app/evaporation_liquid_result.html', {'request': request,
                                                                               'user': user,
                                                                               'zone_array': zipped_list,
                                                                               'plot_div_evaporation': plot_div_evaporation,
                                                                               'calc_data': (
                                                                               molar_mass, strait_area, steam_pressure)
                                                                               })